...
// UnityCG.cginc is already included in MetaAvatarCore.hlsl
...
void GhostBallEffect(float3 WorldSpaceNormal, float3 WorldSpaceViewDirection, float4 ScreenPosition, float2 uv0, out float3 BaseColor, out float Alpha, out float AlphaClipThreshold)
{
...
float4 ApplyGhostEffect(float4 color, float3 worldNormal, float3 worldPos, float4 screenPos, float2 uv)
{
